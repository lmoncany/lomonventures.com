...
2. **Spawn concurrent agents** using Codex's Task tool
3. **Coordinate via hooks** and memory

### 🚨 CRITICAL: CLI + Task Tool in SAME Message

**When user says "spawn swarm" or requests complex work, Codex MUST in ONE message:**
...
Bash("npx @Codex-flow/cli@latest swarm init --topology hierarchical-mesh --max-agents 15")
...
Bash("npx @Codex-flow/cli@latest memory search --query '[task keywords]' --namespace patterns")

# 2. Check if similar task was done before
Bash("npx @Codex-flow/cli@latest memory search --query '[task type]' --namespace tasks")

# 3. Load learned optimizations
Bash("npx @Codex-flow/cli@latest hooks route --task '[task description]'")
```

### After Completing Any Task Successfully
```bash
# 1. Store successful pattern for future reference
Bash("npx @Codex-flow/cli@latest memory store --namespace patterns --key '[pattern-name]' --value '[what worked]'")

# 2. Train neural patterns on the successful approach
Bash("npx @Codex-flow/cli@latest hooks post-edit --file '[main-file]' --train-neural true")

# 3. Record task completion with metrics
Bash("npx @Codex-flow/cli@latest hooks post-task --task-id '[id]' --success true --store-results true")

# 4. Trigger optimization worker if performance-related
Bash("npx @Codex-flow/cli@latest hooks worker dispatch --trigger optimize")
```
...
This project is configured with Codex Flow V3:
...
npx @Codex-flow/cli@latest hooks session-restore --session-id "[id]"

# Intelligence routing
npx @Codex-flow/cli@latest hooks route --task "[task]"
npx @Codex-flow/cli@latest hooks explain --topic "[topic]"

# Neural learning
npx @Codex-flow/cli@latest hooks pretrain --model-type moe --epochs 10
npx @Codex-flow/cli@latest hooks build-agents --agent-types coder,tester

# Background workers
npx @Codex-flow/cli@latest hooks worker list
...
npx @Codex-flow/cli@latest hooks coverage-route --task "[task]"

# Statusline (for Codex integration)
npx @Codex-flow/cli@latest hooks statusline
npx @Codex-flow/cli@latest hooks statusline --json
```
...
npx @Codex-flow/cli@latest migrate status

# Run migration with backup
npx @Codex-flow/cli@latest migrate run --backup

# Rollback if needed
npx @Codex-flow/cli@latest migrate rollback

# Validate migration
npx @Codex-flow/cli@latest migrate validate
```
...
Bash("npx @Codex-flow/cli@latest hooks post-command --command '[operation]' --track-metrics true")

# Periodically run benchmarks (every major feature)
Bash("npx @Codex-flow/cli@latest performance benchmark --suite all")

# Analyze bottlenecks when performance degrades
Bash("npx @Codex-flow/cli@latest performance profile --target '[component]'")
```

### Session Persistence (Cross-Conversation Learning)
```bash
# At session start - restore previous context
Bash("npx @Codex-flow/cli@latest session restore --latest")

# At session end - persist learned patterns
Bash("npx @Codex-flow/cli@latest hooks session-end --generate-summary true --persist-state true --export-metrics true")
```

### Neural Pattern Training
```bash
# Train on successful code patterns
Bash("npx @Codex-flow/cli@latest neural train --pattern-type coordination --epochs 10")

# Predict optimal approach for new tasks
Bash("npx @Codex-flow/cli@latest neural predict --input '[task description]'")

# View learned patterns
Bash("npx @Codex-flow/cli@latest neural patterns --list")
```
...
Run `npx @Codex-flow/cli@latest doctor` to check:
- Node.js version (20+)
- npm version (9+)
- Git installation
...
Codex mcp add ruv-swarm -- npx -y ruv-swarm mcp start  # Optional
Codex mcp add flow-nexus -- npx -y flow-nexus@latest mcp start  # Optional
...
- **Swarm status**: `npx @Codex-flow/cli@latest swarm status`
- **Agent spawn**: `npx @Codex-flow/cli@latest agent spawn -t <type> --name <name>`
- **Memory store**: `npx @Codex-flow/cli@latest memory store --key "mykey" --value "myvalue" --namespace patterns`
- **Memory search**: `npx @Codex-flow/cli@latest memory search --query "search terms"`
- **Memory list**: `npx @Codex-flow/cli@latest memory list --namespace patterns`
- **Memory retrieve**: `npx @Codex-flow/cli@latest memory retrieve --key "mykey" --namespace patterns`
...
npx @Codex-flow/cli@latest memory store --key "bug-fix-123" --value "Fixed null check" --namespace solutions --tags "bugfix,auth"
```
...
npx @Codex-flow/cli@latest memory search --query "authentication patterns"
npx @Codex-flow/cli@latest memory search --query "error handling" --namespace patterns --limit 5
```

### List Entries
```bash
# OPTIONAL: --namespace, --limit
npx @Codex-flow/cli@latest memory list
...
npx @Codex-flow/cli@latest memory retrieve --key "pattern-auth"
...
**KEY**: CLI coordinates the strategy via Bash, Codex's Task tool executes with real agents.
